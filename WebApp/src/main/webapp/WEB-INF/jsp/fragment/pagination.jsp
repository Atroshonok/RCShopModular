...
<nav aria-label="Page navigation">
	<ul class="pagination pagination-sm">
		<li><a href="${pageContext.request.contextPath}/products/all?currentPage=${pagesList[0]}" aria-label="First page"><span aria-hidden="true"><s:message code="pagination.nav.first.page"/></span></a></li>
			<c:choose>
				<c:when test="${activeCurrentPage > 1}">
					<li><a href="${pageContext.request.contextPath}/products/all?currentPage=${activeCurrentPage - 1}" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>
				</c:when>
				<c:otherwise>
			...
			<c:when test="${pagesList.size() >= 7}">
				<c:if test="${activeCurrentPage > 4}"><li><span>...</span></c:if>
					<c:forEach var="i" items="${pagesList}" begin="${startPage}" end="${endPage}" varStatus="status">
						<li id="currentPage${i}"><a href="${pageContext.request.contextPath}/products/all?currentPage=${i}">${i}</a></li>
					</c:forEach>
		...
		<c:choose>
			<c:when test="${activeCurrentPage < pagesList.size()}">
				<li><a href="${pageContext.request.contextPath}/products/all?currentPage=${activeCurrentPage + 1}" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>
			</c:when>
			<c:otherwise>
				<li class="disabled"><span>&raquo;</span></li>
			</c:otherwise>
		</c:choose>
		<li><a href="${pageContext.request.contextPath}/products/all?currentPage=${pagesList.size()}" aria-label="Last page"><span aria-hidden="true"><s:message code="pagination.nav.last.page"/></span></a></li>
	</ul>
</nav>
<script type="text/javascript" src="${pageContext.request.contextPath}/resources/js/pagination_animation.js"></script>